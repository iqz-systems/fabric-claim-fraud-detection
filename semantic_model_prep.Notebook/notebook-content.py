...
# 
# This notebook prepares fact tables for the Power BI semantic model.
# 
# ## Tables Created
# - `publish.frauddetection.fact_claims` - Claims with settlement and fraud indicators
# - `publish.frauddetection.fact_incident_loss_by_product` - Loss aggregations by product
...
# ## Create Fact Claims for Semantic Model
...
logger.info(f"fact_claims saved: {fact_claims.count()} records")
...
# ## Create Incident Loss by Product
...
logger.info(f"fact_incident_loss_by_product saved: {fact_incident_loss_by_product.count()} records")
