"""
================================================================================
NOTEBOOK: 09 - Data Observability - Freshness, Schema, and Volume Monitoring
================================================================================

PURPOSE:
    Monitors data quality across the fraud detection data platform including
    freshness checks, schema drift detection, and volume metrics.
...
WHAT THIS NOTEBOOK DOES:
    ...
    4. Logs all metrics to Application Insights and saves to dataquality tables

PREREQUISITES:
    - Tables created in Raw, Curate, and Publish layers
    - Application Insights configured (optional but recommended)
    - Data quality schema: raw.dataquality
...
HOW TO USE:
    ...
    4. Update configuration:
       ...
       - WORKSPACE_ID and EVENTHOUSE_LAKEHOUSE_ID
       - Table configurations (if needed)
    5. Run all cells

EXECUTION ORDER:
    Can run: Independently or scheduled (e.g., daily)
    Works with: All other notebooks (monitors their output)

OUTPUT TABLES:
    - raw.dataquality.FreshnessChecks (historical freshness data)
    - raw.dataquality.FreshnessChecksLatest (latest snapshot)
    - raw.dataquality.SchemaChangeLog (schema change history)
    - raw.dataquality.VolumeMetrics (volume trends)
...
# # 07 - Data Observability: Freshness, Schema, and Volume Monitoring
# 
# This notebook monitors data quality across all layers of the fraud detection platform.
# 
# **Checks:** Freshness | Schema Drift | Volume Metrics
...
# Application Insights for logging (optional but recommended)
...
print(f"Eventhouse Lakehouse ID: {EVENTHOUSE_LAKEHOUSE_ID}")
...
# ============================================================================
# TABLE CONFIGURATION - Update thresholds as needed
# ============================================================================
...
table_configs = [
    ...
    # Publish Layer
    {"layer": "publish", "table": "publish.frauddetection.fact_claims", "threshold": 2628000, "timestamp_col": "claim_log_date"},
    {"layer": "publish", "table": "publish.frauddetection.claim_settlements", "threshold": 2628000, "timestamp_col": "settlement_date"},
    
    # Eventhouse Tables (real-time) - Uncomment and update if using Eventhouse
    # {"layer": "eventhouse", "table": f"{EVENTHOUSE_BASE_PATH}/claims_electronics_raw/", "threshold": 10080, "timestamp_col": "EventEnqueuedUtcTime"},
]

print(f"Configured {len(table_configs)} tables for monitoring")
...
if freshness_results:
    ...
    print(f"✅ Freshness checks saved: {len(freshness_results)} tables")
...
print(f"✅ Freshness check completed in {duration_sec_freshness} seconds")
...
tables_to_check = [
    ...
    {"layer": "publish", "table": "publish.frauddetection.fact_claims"},
]
...
if schema_changes:
    ...
    print(f"✅ Schema changes saved: {len(schema_changes)} tables")
...
print(f"✅ Schema monitoring completed in {duration_sec_schema} seconds")
...
if volume_results:
    ...
    print(f"✅ Volume metrics saved: {len(volume_results)} tables")

end_time_volume = datetime.utcnow()
duration_sec_volume = round((end_time_volume - start_time_volume).total_seconds(), 2)
logger.info(f"Volume Metrics Monitoring Completed in {duration_sec_volume} seconds")
print(f"✅ Volume monitoring completed in {duration_sec_volume} seconds")
...
print(f"Execution ID: {execution_id}")
print("="*60)
print("\n✅ Data observability monitoring complete!")
print("Check tables:")
print("  - raw.dataquality.FreshnessChecksLatest")
print("  - raw.dataquality.SchemaChangeLog")
print("  - raw.dataquality.VolumeMetrics")
